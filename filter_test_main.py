...
def test_photo(photo_file):
    ...
    hsv = cv2.cvtColor(warped_frame, cv2.COLOR_BGR2HSV)
    canny_image = detecting_edges(warped_frame)
    gray_warped_frame = cv2.cvtColor(warped_frame, cv2.COLOR_RGB2GRAY)
    # cropped_image = region_of_interest(canny_image)
    box_dim = find_contours(canny_image, warped_frame)
    # print(box_dim)
    left_box, right_box = sort_contours(box_dim, warped_frame)
...
# test_photo('Test_Videos_&_Images/blue_lane_picam.png')
# test_photo('Test_Videos_&_Images/blue_lane.jpg')
# test_photo('Test_Videos_&_Images/blue_lane_2.jpg')
